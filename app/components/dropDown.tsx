...
export default function dropDown({ gear, chooseGear }: ImportProp) {
    ...
    return (
        <div className="relative inline-block" ref={clickWatch}>
            <button className="bg-amber-600 btn btn-lg z-100 rounded-2xl" onClick={() => setActive(!active)}>
                v
            </button>

            {active && (
                <ul className={`absolute z-100 left-0 mt-2 w-56 max-h-64 overflow-y-auto bg-neutral-900 text-white rounded-xl shadow-lg py-2 origin-top transition-all duration-200 ease-out
    ${active ? "opacity-100 scale-100 pointer-events-auto" : "opacity-0 scale-95 pointer-events-none"}`}>
                    {gearData[gear].map((item) =>
                        <li key={item.id}>
                            <a onClick={() => { chooseGear(item.id, gear); setActive(false); }} className="flex z-100 items-center gap-3 px-4 py-2 hover:bg-neutral-800 cursor-pointer">
